...
class TestBuildStata(unittest.TestCase):
    ...
    @mock.patch('gslab_scons.build_stata.misc.sys.platform', 'darwin')
    @mock.patch('gslab_scons.build_stata.misc.is_in_path')
    @mock.patch('gslab_scons.subprocess.check_output')
    ...
    def check_output_side_effect(*args, **kwargs):
        ...
        if re.search('stata-mp -e', command):
            # Write a log
            script_name = re.sub('')
            with open('./test_script.log', 'wb'):
                f.write('Test Stata log.\n')
