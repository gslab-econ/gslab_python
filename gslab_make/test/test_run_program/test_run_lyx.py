...
from py.run_program import run_lyx
...
class testRunLyx(unittest.TestCase):
    ...
    def test_custom_log(self):
        ...
        with nostderrout():        
            clear_dirs(output_dir)
            start_make_logging(makelog_file)
            run_lyx(program = './input/lyx_test_file.lyx', makelog = '../output/custom_make.log')
    ...
    def test_comments(self):  
        ...
        with nostderrout():   
            clear_dirs(temp_dir)
            run_lyx(program = './input/lyx_test_file.lyx', comments = True)
        logfile_data = open('../output/make.log', 'rU').read()
        self.assertIn('LaTeX', logfile_data)
    ...
    def test_handout_with_comments(self):  
        ...
        with nostderrout():   
            clear_dirs(temp_dir)
            run_lyx(program = './input/lyx_beamer_test_file.lyx', handout = True, comments = True)
        logfile_data = open('../output/make.log', 'rU').read()
        self.assertIn('LaTeX', logfile_data)
    ...
    def test_handout_change_dir(self):
        ...
        with nostderrout():    
            clear_dirs(temp_dir)
            run_lyx(program = './input/lyx_test_file.lyx', handout = True, changedir = True)
        logfile_data = open('../output/make.log', 'rU').read()
        self.assertIn('LaTeX', logfile_data)
    ...
    def test_handout_pdfout(self):
        ...
        with nostderrout():    
            clear_dirs(temp_dir)
            run_lyx(program = './input/lyx_test_file.lyx', handout = True, pdfout = './input/custom_outfile.pdf')
        logfile_data = open('../output/make.log', 'rU').read()
        self.assertIn('LaTeX', logfile_data)
        self.assertTrue(os.path.isfile('./input/custom_outfile.pdf'))
