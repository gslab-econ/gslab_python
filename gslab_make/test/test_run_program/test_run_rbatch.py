...
from py.run_program import run_rbatch
...
class testRunRBatch(unittest.TestCase):
    ...
    def test_custom_log(self):
        ...
        with nostderrout():
            clear_dirs(output_dir)
            start_make_logging(makelog_file)
            run_rbatch(program = './input/R_test_script.R', makelog = '../output/custom_make.log')
    ...
    def test_executable(self):
        with nostderrout():
            run_rbatch(program = './input/R_test_script.R', executable = 'R CMD BATCH') 
        logfile_data = open('../output/make.log', 'rU').read()
        self.assertIn('Test script complete', logfile_data)
        self.assertTrue(os.path.isfile('output.txt'))
    ...
    def test_bad_executable(self):
        with nostderrout():
            run_rbatch(program = './input/R_test_script.R', executable = 'nonexistent_R_executable')
        logfile_data = open('../output/make.log', 'rU').read()
        if os.name == 'posix':
    ...
    def test_two_option(self):
        with nostderrout():
            run_rbatch(program = './input/R_test_script.R', option = '--no-timing --slave')
        logfile_data = open('../output/make.log', 'rU').read()
        self.assertIn('Test script complete', logfile_data)
