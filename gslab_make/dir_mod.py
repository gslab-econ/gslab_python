...
def check_manifest(manifestlog = '@DEFAULTVALUE@',
    ...
    or .csv files not in the manifest along with a list of these files. All log is printed
    to "makelog" log file."""
...
def clear_dirs(*args):
    """Create a directory for each argument if they do not already exist.
    Delete all files in the directory keeping directory structure
    (i.e. sub-directories)."""
    # metadata.settings should not be part of argument defaults so that they can be
    # overwritten by make_log.set_option  
    for dir in args:
