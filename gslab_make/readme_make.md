...
Prerequisites:
(1) Python 2.7 installed and executable path is added to system path
(2) Matlab installed and and executable path is added to system path
(3) Stata installed and executable defined as an environment variable / symbolic link
(4) Perl installed and executable path is added to system path
(5) Mathematica 8.0 installed and math kernel path is added to system path
(6) StatTransfer installed and executable path is added to system path
(7) Lyx installed and executable path is added to system path
(8) R installed and executable path is added to system path
(9) SAS installed and executable path is added to system path
(10) Requires SVN Version 1.7 or higher. See ‘clear_dirs’ command below for reasoning.
...
* [bool_comments] is a Boolean (True/False) value to specify if we want to print out the lyx notes when compiling 
a lyx file using run_lyx().
...
by set options **]. All other functions that come after it will not have to re-specify those options.
