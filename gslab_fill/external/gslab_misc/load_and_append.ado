 ...
 program load_and_append
    ...
    * Load and append
	...
	foreach i of numlist 2/`numfiles' {
		...
		if `"`inputfile`i''"'!="" {
			qui replace `dsnamet'=`"`inputfile`i''"' in `nobsp'/l
		}
		if `"`obsseq'"'!="" {
    ...
    foreach rawname in `anything' {
        ...
        * Handle wildcards
        if strpos("`rawfilename'", "*") > 0 {
            local dir "."
            if "`path'"~="" local dir "`path'"
            local newfiles : dir `"`dir'"' files `"`rawfilename'"', respectcase
        }
        else {
